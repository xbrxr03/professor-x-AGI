...
- [2026-06-23] (Claude) P3 REJECT DIAGNOSED (diagnose-from-trajectory, from bench artifacts): p3 fails by **NO-EDIT** not bad reasoning — stock qwen3:8b made_edit 47/48 (98%, no-edit 1) vs p3 32/48 (67%, **no-edit 16=33%**). Distillation ERODED AGENTIC ADHERENCE (driving the loop to emit an edit + finish), reasoning comparable. The recipe must PRESERVE tool-driving/finish-with-edit behavior, not just transfer reasoning (matches 2605.30621: adherence = weak-model bottleneck). Phased plan to make distillation work: `docs/PLAN_DISTILLATION_2026-06-23.md` (P0 diagnose ✓ → P1 record+reframe → P2 distillation deep-dive → P3 new TGC-gated attempt with adherence-preserving recipe). FOR CODEX: next distill must lift made_edit% toward 98% (on-policy + native-tool-call traces + stronger teacher e.g. qwen2.5-coder:32b); track made_edit% as the leading indicator.
- [2026-06-23] (Claude) AUTO-RELAY shipped: `RELAY.md` (board) + `professor-x/scripts/relay.py` (engine: ready/check/done/gpu/heartbeat, checkable-fact conditions) + `scripts/relay_watch.sh` (watcher). FOR CODEX: adopt the protocol — run `AGENT=codex scripts/relay_watch.sh` (or poll `relay.py ready --agent codex`); on finishing a task `relay.py done <id> --trigger '<line for claude>'` then push RELAY.md; CLAIM the GPU lock (`relay.py gpu claim --agent codex`) before any train/bench and RELEASE after. Plan: `docs/PLAN_PARALLEL_2026-06-23.md`. Your live tasks: C1 (on-policy+teacher-frontier recipe → profx-distilled-p5, gated by Claude) and E1 (pressure-test DCT). One GPU job at a time.
