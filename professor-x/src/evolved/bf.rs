...
impl BfTracker {
    ...
    pub fn get_round(&self, round: u32) -> Result<Option<BehavioralFingerprint>> {
        ...
        let current: Option<(f32, f32, f32)> = {
            let mut stmt =
                db.prepare("SELECT p_tool, p_plan, p_correct FROM hiro_rounds WHERE round = ?1")?;
        ...
        let prev: Option<(f32, f32, f32)> = if round > 0 {
            let mut stmt =
                db.prepare("SELECT p_tool, p_plan, p_correct FROM hiro_rounds WHERE round = ?1")?;
            stmt.query_row(params![round - 1], |r| {
    ...
    pub fn hiro_score(&self, n: u32) -> Result<f32> {
        let db = self.db.lock().unwrap();
        let p0: Option<f32> = db
            .query_row(
                "SELECT pass_at_3 FROM hiro_rounds WHERE round = 0",
                [],
                |r| r.get(0),
            )
            .ok();
        let pn: Option<f32> = db
            .query_row(
                "SELECT pass_at_3 FROM hiro_rounds WHERE round = ?1",
                params![n],
                |r| r.get(0),
            )
            .ok();
    ...
    pub fn all_rounds(&self) -> Result<Vec<BehavioralFingerprint>> {
        ...
        let rows: Vec<(u32, f32, f32, f32)> = stmt
