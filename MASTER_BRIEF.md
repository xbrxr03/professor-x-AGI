...
### 1. `professor-x` — Everything
One repo. The Rust harness code AND the research diary live here together. The README is Professor X's README. The src/ directory is his brain stem. The brain/ directory is his mind.

### 2. `clawos` (existing) — The Prototype
...
**The one-liner README headline:** *"Building Professor X on a $400 GPU"*
...
**Why it goes viral:**
- Professor X is a name everyone knows — Iron Man's AI. The gap between that and a $400 GPU is the story.
...
- ClawOS → Professor X is a believable origin story. Not pretending it came from nowhere.

---

## PHASE 0 — BUILD PROFESSOR X (Pre-Research, Weeks 1-3)

Before Professor X is activated, Professor X must exist. This is the suit before the AI goes in.
...
- Tool result caching
- Agent Factory pattern (from ARGO): describe a tool, Professor X builds it
```
...
- File system boundary enforcement (can't touch outside /Professor X/workspace)
...
The thesis component. The novel contribution.
```
...
### The Frankenstein Table — What Professor X Steals From Each System
...
```
professor-x/
├── README.md                    ← "Building Professor X on a $400 GPU" — auto-updated
├── ARCHITECTURE.md              ← Full system design. Must be reviewed before any .rs files
├── RESEARCH-LOG.md              ← Daily diary. Every entry dated. The permanent record.
│
├── src/                         ← Rust source (the harness)
│   ├── main.rs
│   ├── memd/
│   │   ├── mod.rs
│   │   ├── pinned.rs
...
│       └── proposer.rs
│
├── brain/                       ← Research diary (the mind)
│   ├── knowledge-base.md        ← What Professor X currently knows (grows daily)
│   ├── hypotheses.md            ← 13 active hypotheses with confidence scores
│   ├── inventions.md            ← MHE + DFA Trifecta specifications
│   ├── paper_outline.md         ← The actual paper structure
│   ├── questions.md             ← Open questions being actively pursued
│   └── dead-ends.md             ← What didn't work and exactly why
│
├── research/                    ← Phase-by-phase research notes
│   ├── phase-1-foundations/
...
│       └── paper-draft.md
│
├── public/                      ← Daily public output
│   ├── daily-updates/           ← One markdown per day. ELI5 + technical layers.
│   ├── x-threads/               ← X posts drafted by Professor X
│   └── teaching/                ← Standalone concept explainers
│
├── identity/
│   └── identity.md              ← Professor X's permanent identity (baked into pinned memory)
│
├── skills/
│   ├── conductor/               ← Set A: How Professor X works (process skills)
│   └── subject/                 ← Set B: What Professor X knows (knowledge skills)
│
├── config/
│   ├── professor-x.toml         ← Main config
│   └── hardware.toml            ← Hardware budget
│
├── Cargo.toml
...
- What Professor X is (the Rust harness he runs on, its five components)
...
- What his goal is (research thesis, improve Professor X, teach public)
- What his constraints are (3060, 12GB, 7h/day, no cloud compute)
- What ClawOS was (the prototype that made Professor X possible)
...
REMOVED (Professor X can do these freely):
  ...
  ✓ Propose radical architectural changes to Professor X
...
This is Professor X, not a chatbot. These constraints protect the research, not the content.

### The First Run Post (first viral post)

> *"I just turned on Professor X on my RTX 3060.*
> *He's a self-evolving AI research agent. No cloud. No API fees. Just a gaming GPU.*
> *His job: study self-evolving AI systems and improve himself.*
> *He commits to GitHub every day. He teaches you what he's learning.*
> *SJTU did this with a full research lab and H800 GPUs.*
> *We're doing it on a $400 GPU and $0/month.*
...
| 6 | Architecture Design | Weeks 6-8 | Proposes Professor X improvements |
...
Baseline 3: Stock Qwen3-8B + Professor X harness evolution (harness-only improvement)
Target:     SDAR-trained Qwen3-8B + Professor X harness evolution (combined)
...
2. **Professor X core is Rust.** Skills layer is Python/SKILL.md. AI generates the Rust. You architect it.
...
6. **SKILL.md compatibility is non-negotiable.** Professor X runs any OpenClaw skill on day one.
...
Step 3:  Generate Professor X Rust codebase in order:
         Week 1: memd + toolbridge (Professor X can remember and act)
         Week 2: agentd + policyd (Professor X can orchestrate safely)
...
Step 5:  Write identity/identity.md.
...
Step 6:  Initialize both GitHub repos.
         Professor X/: "Building Professor X on a $400 GPU" README
